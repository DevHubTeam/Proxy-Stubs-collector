...
export class StubsGenerator {
    ...
    async save() {
        ...
        const configInstance = new StubsConfigUtil(
            this.url,
            urlHash,
            this.scenarioName,
            `stubs_folder/${this.scenarioName}`
        );
    ...
    saveBodyFile = (urlHash, content) => this.stubsFileUtils.saveBodyFile(urlHash, content);

    setUpFolder = () => this.stubsFileUtils.setUpFolder();
    getUrlHash = () => hashString(this.url);
}
